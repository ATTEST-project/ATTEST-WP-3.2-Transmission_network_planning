...
active_line_flow_normal_lines=@expression(model_name, [ t in 1:nTP, b in 1:nBus, j in node_data[b].node_cnode; ~isempty(node_data[b].node_num)], value.(pinj_dict[[t,b,j]]))
active_line_flow_normal_trans=@expression(model_name, [ t in 1:nTP, b in 1:nBus, j in node_data_trans[b,1].node_cnode; ~isempty(node_data_trans[b,1].node_num)], value.(pinj_dict[[t,b,j]]))
active_line_flow_normal=Dict{NTuple{3,Int64},Float64}()
...
reactive_line_flow_normal_lines=@expression(model_name, [ t in 1:nTP, b in 1:nBus, j in node_data[b].node_cnode; ~isempty(node_data[b].node_num)], value.(qinj_dict[[t,b,j]]))
reactive_line_flow_normal_trans=@expression(model_name, [ t in 1:nTP, b in 1:nBus, j in node_data_trans[b,1].node_cnode; ~isempty(node_data_trans[b,1].node_num)], value.(qinj_dict[[t,b,j]]))
reactive_line_flow_normal=Dict{NTuple{3,Int64},Float64}()
...
# active_line_flow_contin_lines=@expression(model_name, [c in 1:nCont,t in 1:nTP, b in 1:nBus, j in node_data_contin[c][b].node_cnode_c; ~isempty(node_data_contin[c][b].node_num_c)], value.(pinj_dict_c[[c,1,1,b,j]]))
# active_line_flow_contin_trans=@expression(model_name, [c in 1:nCont,t in 1:nTP, b in 1:nBus, j in node_data_trans[b,1].node_cnode;     ~isempty(node_data_trans[b,1].node_num)],     value.(pinj_dict_c[[c,1,1,b,j]]))
...
for t in 1:nTP, i in 1:nLines
    if line_flow_normal_dual_r[t,i]>line_flow_normal_dual_s[t,i]   #maximum value between i-j and j-i
        replace(line_flow_normal_dual_s, line_flow_normal_dual_s[t,i]=>line_flow_normal_dual_r[t,i])
    end
end
...
for t in 1:nTP, i in 1:nTrans
    if line_flow_normal_trans_dual_r[t,i]>line_flow_normal_trans_dual_s[t,i]    #maximum value between i-j and j-i
        replace(line_flow_normal_trans_dual_s, line_flow_normal_trans_dual_s[t,i]=>line_flow_normal_trans_dual_r[t,i])
    end
end

line_flow_normal_dict=Dict{NTuple{3,Int64},Float64}()
for t in 1:nTP, i in 1:nLines
    push!(line_flow_normal_dict, (t,idx_from_line[i],idx_to_line[i])=>line_flow_normal_dual_s[t,i])
end

for t in 1:nTP, i in 1:nTrans
    push!(line_flow_normal_dict, (t,idx_from_trans[i],idx_to_trans[i])=>line_flow_normal_trans_dual_s[t,i])
end
...
# line_flow_normal_trans_dual_s=JuMP.dual.(line_flow_contin_trans_s)
# line_flow_normal_trans_dual_r=JuMP.dual.(line_flow_contin_trans_r)
# for i in eachindex(line_flow_normal_trans_dual_s)    #maximum value between i-j and j-i
#     if line_flow_normal_trans_dual_r[i]>line_flow_normal_trans_dual_s[i]
...
#     push!(line_flow_contin_dict, (c,idx_from_trans[i],idx_to_trans[i])=>line_flow_normal_trans_dual_s[c,1,1,i])
# end
...
active_power_balance_normal_dual_dict=Dict{NTuple{2,Int64},Float64}()
for t in 1:nTP, i in 1:nBus
    push!(active_power_balance_normal_dual_dict, (t,i)=>active_power_balance_normal_dual[i])
end
...
reactive_power_balance_normal_dual_dict=Dict{NTuple{2,Int64},Float64}()
for t in 1:nTP, i in 1:nBus
    push!(reactive_power_balance_normal_dual_dict, (t,i)=>reactive_power_balance_normal_dual[i])
end
