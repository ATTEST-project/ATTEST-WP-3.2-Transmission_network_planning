...
#     sheetname  = "Loads";
#     fields  = ["Bus_N",	"ShFr",	"ShFrMin",	"ShFrMax",	"ShFrOK",	"ShCost" ]; # Fields that have to be read from the file
#     rflex_data = ods_readall(filename_prof;sheetsNames=[sheetname],innerType="Matrix")
#     raw_flex_data = rflex_data[sheetname]   # Conversion from Dict to Array
#
#     flex_data  = raw_flex_data[2:end,:]                  # Buses order in load sheet
#
#     idx_floads = []                                                                  # Indices of parallel loads
#     for i in 1:size(flex_data,1)
#         # gens = data[i,1]
#         ploads = flex_data[:,1].-flex_data[i,1]
...
#         if isempty(idx_floads)
#             push!(idx_floads,id_ploads)
#         elseif isempty(findall(x->x==id_ploads,idx_floads))
#             push!(idx_floads,id_ploads)
#         else
#             # do nothing!
#         end
#     end
#     new_fload_data = zeros(size(idx_floads,1),size(flex_data[1,:],1) )
#     for i in 1:size(idx_floads,1)
#         iloads = idx_floads[i,1]
#         # pavail=sum(data[iloads,2:end],dims=1)
#         new_fload_data[i,1]=flex_data[idx_floads[i][1],1]
#         new_fload_data[i,2:end]=flex_data[idx_floads[i][1],2:end]
#     end
#
#
# # bus_data_lsheet       = new_fload_data[:,1]
# # bus_data_lsheet       = nw_pPrf_data_load[:,1]
# cost_flex_load        = new_fload_data[2:end,6]
# # idx_Gs_lsheet = findall3(x->x=="Gs (MW)",rheader_loads)
# # idx_Bs_lsheet = findall3(x->x=="Bs (MVAr)",rheader_loads)
# idx_St_lsheet = findall3(x->x=="ShFrOK",raw_flex_data[1,:])                         # Index of status in load sheet!
#
# iFl     =  findall3(x->x==1,new_fload_data[:,idx_St_lsheet])                           # Index of flexible loads
# nFl     =  size(iFl,1)                   # Number of flexible loads in a system
# nd_fl   =  convert.(Int64,new_fload_data[findall3(x->x==1,new_fload_data[:,idx_St_lsheet]),1]) # Nodes to which flexible loads are connected. Node data is taken from the power profile sheet.
# rdata_loads_bus=convert.(Int64,new_fload_data[1:end,1])
# nd_fl_bus   =  [indexin(nd_fl[i],rdata_loads_bus)[1] for i in 1:nFl if nFl!=0]

# return cost_flex_load,idx_St_lsheet,iFl,nFl,nd_fl,nd_fl_bus
cost_flex_load = fl_cost
# nFl=nEV
# nd_fl=nd_EV

return cost_flex_load,nFl,nd_fl
end
