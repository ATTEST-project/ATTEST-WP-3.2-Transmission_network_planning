...
sheetname="Dimension"
fields  = ["NTP","NSC", "RES_MAG","Load_MAG","Line_MAG", "PF"]
raw_data = ods_readall(filename;sheetsNames=[sheetname],innerType="Matrix")
raw_data = raw_data[sheetname]   # Conversion from Dict to Array
header    = raw_data[1,:]
nTP      = raw_data[2,1]
nSc      = raw_data[2,2]
RES_MAG  = raw_data[2,3]
Load_MAG  = raw_data[2,4]
Line_MAG  = raw_data[2,5]
power_factor = raw_data[2,6]
raw_data = nothing
header = nothing
data = nothing
...
header = nothing
data = nothing
#------------------------ Formatting the Flexible-----------------------------
sheetname="FL"
# sheetname  = "Stor_info";
fl_info   = ods_readall(filename;sheetsNames=[sheetname],innerType="Matrix")
fl_info   = fl_info[sheetname]
idx_fl_bus=findall3(x->x=="Bus", fl_info[1,:])
idx_fl_percent=findall3(x->x=="percent", fl_info[1,:])
idx_fl_cost=findall3(x->x=="cost", fl_info[1,:])
nd_fl=fl_info[2:end,idx_fl_bus]
nd_fl=vec(nd_fl)
nFl=length(fl_info[2:end,1])
if nFl==0
    nd_fl=[]
end

percent=fl_info[2:end,idx_fl_percent]  #values are in %
fl_cost=fl_info[2:end,idx_fl_cost]
# sheetname  = "Loads";
# fields  = ["Bus_N","Pd (pu)","Qd (pu)","ShFr","ShFrMin","ShFrMax","ShFrOK","ShCost"]; # Fields that have to be read from the file
# raw_data = ods_readall(filename;sheetsNames=[sheetname],innerType="Matrix")
# raw_data = raw_data[sheetname]   # Conversion from Dict to Array
#
# header    = raw_data[1,:]
