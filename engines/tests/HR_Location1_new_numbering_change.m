...
new_numbering_path = 'C:\Users\m36330ac\Documents\MEGA\Eduardo Alejandro Martinez Cesena\WP3\Python\from Wangwei\NEW\ATTEST_Tool3.2\engines\tests\HR_Location1_numbering.xlsx';
new_numbering = table2array(readtable(new_numbering_path))

buses = size(mpc.bus);
Nbus = buses(1)
branches = size(mpc.branch);
Nbranch = branches(1)
gens = size(mpc.gen);
Ngen = gens(1)
...
for i_bus = 1:Nbus
    old_number = new_mpc.bus(i_bus,1);
    position = find(new_numbering(:,1) == old_number);
    new_number = new_numbering(position,2);
    new_mpc.bus(i_bus,1) = new_number;
end

for i_gen = 1:Ngen
    old_number = new_mpc.gen(i_gen,1);
    position = find(new_numbering(:,1) == old_number);
    new_number = new_numbering(position,2);
    new_mpc.gen(i_gen,1) = new_number;
end

for i_branch = 1:Nbranch
    old_number1 = new_mpc.branch(i_branch,1);
    position = find(new_numbering(:,1) == old_number1);
    new_number = new_numbering(position,2);
    new_mpc.branch(i_branch,1) = new_number;
    
    old_number2 = new_mpc.branch(i_branch,2);
    position = find(new_numbering(:,1) == old_number2);
    new_number = new_numbering(position,2);
    new_mpc.branch(i_branch,2) = new_number;
