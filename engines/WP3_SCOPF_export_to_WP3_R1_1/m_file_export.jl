...
flex_array_inc[1][b]=JuMP.value.(p_fl_inc[1,1,idx_flex[1]])
flex_array_dec[1][b]=JuMP.value.(p_fl_dec[1,1,idx_flex[1]])
end
end
end
...
for b in 1:nBus
    for i in nd_fl
        if i==b
            idx_flex=findall(x->x==i, nd_fl )
flex_array_inc_q[1][b]=JuMP.value.(q_fl_inc[1,1,idx_flex[1]])
flex_array_dec_q[1][b]=JuMP.value.(q_fl_dec[1,1,idx_flex[1]])
end
end
end
...
active_load_curt[1][b]=JuMP.value.(pen_lsh[1,1,idx_lc[1]])
# flex_array_dec[1][b]=JuMP.value.(p_fl_dec[1,1,idx_flex[1]])
end
end
end
...
for b in 1:nBus
    for i in bus_data_lsheet
        if i==b
            idx_lc=findall(x->x==i, bus_data_lsheet )
reactive_load_curt[1][b]=JuMP.value.(tan(acos(power_factor))*pen_lsh[1,1,idx_lc[1]])
# flex_array_dec[1][b]=JuMP.value.(p_fl_dec[1,1,idx_flex[1]])
end
end
end
...
pgen[1][b]=JuMP.value.(Pg[1,1,idx_pg[1]])
# flex_array_dec[1][b]=JuMP.value.(p_fl_dec[1,1,idx_flex[1]])
end
end
...
pgen_neg[1][b]=JuMP.value.(Pg_neg[1,1,idx_pg_neg[1]])
# flex_array_dec[1][b]=JuMP.value.(p_fl_dec[1,1,idx_flex[1]])
end
end
end
...
qgen[1][b]=JuMP.value.(Qg[1,1,idx_qg[1]])
# flex_array_dec[1][b]=JuMP.value.(p_fl_dec[1,1,idx_flex[1]])
end
end
if haskey(new_data, "negGen")
...
qgen_neg[1][b]=JuMP.value.(Qg_neg[1,1,idx_qg_neg[1]])
# flex_array_dec[1][b]=JuMP.value.(p_fl_dec[1,1,idx_flex[1]])
end
end
end
...
active_poewr_balance_contin_dual = [active_poewr_balance_contin_dual[r,:] for r in 1:size(active_poewr_balance_contin_dual,1)]
reactive_poewr_balance_contin_dual = [reactive_poewr_balance_contin_dual[r,:] for r in 1:size(reactive_poewr_balance_contin_dual,1)]
...
# load_curtailment_n=[JuMP.value.(pen_lsh[1,1,:])]
# load_curtailment_c=[JuMP.value.(pen_lsh_c[:,1,1,:])]
s="{ \"cont_list\" : $lines,
      \"OPF_bra\" : $flow_normal_final,
      \"OPF_bra_con\" : $flow_contin1,
      \"OPF_Pbal\" :$active_poewr_balance_normal_dual,
      \"OPF_Pbal_con\" :$active_poewr_balance_contin_dual,
      \"OPF_Qbal\" :$reactive_poewr_balance_normal_dual,
      \"OPF_Qbal_con\" : $reactive_poewr_balance_contin_dual,
